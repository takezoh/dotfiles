...
- 現在有効な永続設計は `docs/design/`、採用判断は `docs/adr/`、移行記録や調査メモは `docs/note/` に置く。
- root と module の README は導入・操作手順の正本として維持し、design 文書へ command 一覧や keymap inventory を複製しない。
- `external/` の文書は upstream submodule 所有であり、dev-docs へ取り込まない。
- frontmatter、relation、status は可能な限り dev-docs CLI で更新し、編集後に docs lint を実行する。
