...
class RuntimeBoundaryTests(unittest.TestCase):
	def test_credproxy_does_not_package_context_service(self):
		...
		production.extend(path for path in (MODULE / "assets").rglob("*") if path.is_file())
	...
	def test_host_profiles_order_runtime_before_proxy(self):
		for name in ("host-wsl.sh", "host-darwin.sh", "host-ubuntu-server.sh"):
			...
			self.assertLess(text.index("agent-module"), text.index("context-fabric-service"), name)
			self.assertLess(text.index("context-fabric-service"), text.index("credproxy"), name)
