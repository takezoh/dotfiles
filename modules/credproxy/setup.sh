#!/usr/bin/env bash
# Package the fail-closed credential authority. Each host retrieves the stored
...
trusted_runtime_ready() {
	...
	for path in "$RUNTIME_ROOT/bin/credproxyd" "$RUNTIME_ROOT/bin/credproxy" \
		"$RUNTIME_ROOT/hooks/onepassword-resolve"; do
...
if is_darwin; then
	...
	if bash "$MODULE_DIR/provision-service-account-token.sh"; then
		...
		log "credproxy: launchd enabled with protected service-account token authority"
	else
		launchctl bootout "gui/$(id -u)/com.takezoh.credproxyd" >/dev/null 2>&1 || true
		log "credproxy: credential_source_unavailable (protected service-account token absent); route disabled"
	fi
	exit 0
fi
...
# Remove only obsolete dotfiles-managed drop-ins. The daemon needs neither a
# credential environment nor a Windows CLI PATH.
rm -f "$UNIT_DIR/credproxyd.service.d/20-credential-source.conf" \
	"$UNIT_DIR/credproxyd.service.d/30-wsl-op-path.conf"
